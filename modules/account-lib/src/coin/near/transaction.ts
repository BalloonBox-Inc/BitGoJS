...
export class Transaction extends BaseTransaction {
  ...
  toBroadcastFormat(): string {
    ...
    if (this._nearSignedTransaction) {
      return nearAPI.utils.serialize.base_encode(
        nearAPI.utils.serialize.serialize(nearAPI.transactions.SCHEMA, this._nearSignedTransaction),
      );
    } else {
      return nearAPI.utils.serialize.base_encode(
        nearAPI.utils.serialize.serialize(nearAPI.transactions.SCHEMA, this._nearTransaction),
      );
    }
  }
  ...
  sign(signer: KeyPair): void {
    ...
    const signature = signer.signMessage(Buffer.from(serializedTxHash).toString('hex'));
